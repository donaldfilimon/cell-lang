...
- `arc` retain/release IS present in the C backend and only there: boxing for a literal or call result, `cell_arc_clone` at OWNERSHIP.md R11's retain-a-place sites, the deliberate non-retain for a `shared` parameter, and the drop pass for release. `examples/arc.cell` compiles, links against `examples/arc_host.c`, runs, and prints a strong count.
  ...
  **NOT memory-safety, and counted separately because they fail differently: THREE.** A list literal took its element C type from the first element rather than the declared type, so `[String]` could hold `cell_arc_t` and a callee read a refcount box pointer as a length: silent, ASan-clean, and it PREDATED this work. The LLVM backend passed a spilled copy for an `exclusive` parameter, so a write-through printed 37 where C printed 42: silent, and two separate bugs, one per side of the call, either of which alone still printed 37. And whole-value assignment through an `exclusive` borrow emitted C that does not compile: loud, therefore the safe one.

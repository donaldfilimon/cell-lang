...
./zig-out/bin/cell test tests                        # run a directory of programs through the run recipe
...
**Full gate**, fifteen stages plus a verdict, in the order `tools/check.sh` prints them: build, tests, corpus, backend agreement, mlir lowering, execution, leaks, backend answers, sanitized execution, declared signatures, rule lists, cli build/run, prelude signatures, cli test, grok bots. `grep -E '^== '` on a gate log is the authority for this list:
...
- Drop insertion IS present in the C backend as of `7eaca7a`, and only there: `codegen.zig` emits scope drops, deliberately conservative (a `shared` or `copy` parameter, a match-arm binding and an R3a revival are never dropped; an `owned` or `arc` parameter is, since R11 row 1 closed on 2026-09-16, and a `record` with owning fields is, through its drop glue) and written to fail toward a leak rather than a double free. The LLVM and MLIR backends emit no drops.
- `arc` retain/release IS present in the C backend and only there: boxing for a literal or call result, `cell_arc_clone` at OWNERSHIP.md R11's retain-a-place sites, the deliberate non-retain for a `shared` parameter, and the drop pass for release. `examples/arc.cell` compiles, links against `examples/arc_host.c`, runs, and prints a strong count.
  ...
  **Leak gaps: NONE remain in R11's table (2026-09-16).** The last row, an `owned` String or list PLACE bound as `arc`, was a refusal since `c6ddda3` and is implemented at `let`, at a direct `-> arc T` return, by assignment into a whole `var arc`, as an argument to an `arc` parameter and into a struct literal's `arc` field, for a whole binding (the source is moved into the box; `examples/leaks/arc_box_move.cell` pins all five at 0); other sources and positions stay refused as not implemented, and a move on only one branch leaks by design. An `arc` parameter was never released by a Cell body (no parameter was dropped) until 2026-09-16, when R11 row 1 closed: `owned` and `arc` parameters enter the drop pass, a returned `arc` parameter is retained like any local, and the fixture is pinned at 0. A struct with an `arc` field was never dropped until 2026-09-15 late night, when per-struct drop glue closed R11 row 2 (pinned at 0; two borrowck refusals landed first as preconditions, R12's `copy` binding/parameter clause and R2's list-element clause, the latter closing a measured live heap-use-after-free for `String`). A sixth, an unbound `arc` temporary dropping its handle on the floor, was CLOSED in `460b9a3` and now measures 0, and the block-scoped `arc` local (unbounded in a `while` body) was CLOSED on 2026-09-15 by block-scoped release in `codegen/stmts.zig`'s `emitStmts`, pinned at 0 in the same stage. A SEVENTH, never in the table, was `let owned ys: [Int] = fresh()` and `ys = fresh()` over an `arc`-returning callee, which emitted no drop and leaked the box; it is closed **by refusal** rather than by a release, so it gets no fixture and changes no gate constant. **The authority for these numbers is `tools/check.sh`'s `== leaks ==` stage**, which pins each against a fixture under `examples/leaks/` as a constant carrying the commit it was measured at. Prose here is not the authority; the gate is. `docs/OWNERSHIP.md` R11 says what each gap IS.
  ...
  **A TENTH is now FIXED, and the shape of the fix differs per backend on purpose.** The `str` -> owning-`String` conversion does not exist outside the `arc` direction. A string literal is a 16-byte borrowed `cell_str_t`; an owned `String` is a 24-byte `cell_string_t`; converting between them needs `cell_string_from_str`, which copies the characters. `emitArcConversion` returns false the instant `want.shape != .arc`, so **eight positions converted nothing**: `return` from `-> String`, a `let owned` initializer, a `var owned` initializer, an `owned` call argument, a struct-literal field, an assignment's right side, and -- found only by the implementer, in no brief -- a value-slot `match` arm at a `let` and the same at a `return`, which reach `emitValueInto` rather than the `emitArgLike` path the other six share, so a six-row patch would have left both broken. **RESOLVED 2026-09-08.** The C backend now CONVERTS through one funnel (`emitConversion` -> `emitOwningStringConversion`, emitting `cell_string_from_str`), because C can call that helper and is the reference backend that lowers the whole language (*2026-09-21: still the only backend that lowers the whole language, but no longer the primary direction; see Codegen and backends*); LLVM and MLIR REFUSE all of them (`3e60123`, `e19478c`), because neither can call a `static inline` runtime function (*that reason was wrong, corrected 2026-09-17: `cell_string_from_str` is a real symbol*). The three backends disagreed on these programs by design until IR String step (a) (2026-09-17), when LLVM and MLIR began converting at the same eight positions; see *Codegen and backends*. The safety argument is structural rather than case-by-case, which is why it covers sources nobody enumerated: the conversion COPIES, so it takes no ownership from its source, and nothing whose C type is `cell_str_t` is ever freed by this backend (`hasDropCall(.str)` is false and `emitDropFor` has no `.str` spelling). An already-owned `String` place is structurally unreachable by the predicate and still emits exactly one free, which is the double-free hazard that had to be got right. `examples/owned_string.cell` exercises all eight positions and prints 44, pinned by both an `EXPECT-OUTPUT` line and a `run_c_host` row, closing the corpus hole that hid this: **no file returned a `String` from a body**. All six pass `cell check` with exit 0 and all six emit C that fails `-Wall -Wextra -Werror`. Measured again at `1dd0e6b` after wave 5's let-binding fix landed, which closes none of them: it is a different root cause, since the declared type is already correct and it is the INITIALIZER that is unconverted. **C is the safe backend here and LLVM is the unsafe one**, which inverts the usual reading: C refuses to compile all six, while `llvmemit.zig` and `mlirmit.zig` call zero runtime string constructors between them (`grep -c` returns 0 in both) and instead emit `alloca %cell_string` followed by `store %cell_str`, writing 16 bytes into a 24-byte slot, leaving `cap` uninitialized and `.ptr` aimed at a static literal. That is not a live use-after-free today only because neither backend emits drops; it becomes a free of a non-heap pointer the moment they do. Both backends are documented to refuse rather than emit plausible wrong code, so this is a breach of their own contract, and `let arc s: String = "ab"` is ALREADY refused by both -- the siblings were simply never enumerated. **The gate cannot see any of it: no corpus file returns a `String` from a body**, `examples/primitives.cell:33` only declares one bodyless, so the whole-corpus `-Werror` stage never compiles a str-into-String conversion, and a sanitizer would not help either because the corpus does not exercise the shape.

  **An ELEVENTH is FIXED (2026-09-16), and it was found by review, not by the gate.** R2.a was checked at one point, the fall-through end of a `while` body, so every other path out of the body escaped it. Five shapes were accepted by `cell check` (exit 0) and ran as AddressSanitizer double frees (exit 134) at `05116dc`: a `continue` between a move and its revival; a `break` there followed by a use after the loop; a condition move followed by a use after the loop; a revival of a place moved before the loop, whose `swapRemove` hid a body move below the index the body-end check started from; and a use after a loop whose body may run zero times. The emitted C had no free of the moved place, so the fault was borrowck's, and four documents described only the leak half. The brief named the first three (with nested and condition variants, six probes in all); the last two were found by measurement during the fix. It is now refused by R2.a's jump clause (`docs/OWNERSHIP.md` R2.a, `examples/rejected/skip_revival_jump.cell`), conservatively: a `continue` after a move is refused even when the next iteration reassigns before reading, by Donald's ruling, pending a precise divergence walk.
...
- Generics, enum payloads, and loops other than `while` are not present. `Result<T,E>` is present for scalar payloads, as per-pair structs (cell_rt.h ABI 2, SPEC 3.4), and for an owning `String` on either side in the C backend only (`Ok(owned s)`/`Ok(shared s)`/`Err(owned e)`/`Err(shared e)` patterns, 2026-09-17); `String?` is an owning optional in C (`Some(owned s)`/`Some(shared s)`). NLL is present for named loans only, with the boundary stated above; nothing shortens a temporary loan beyond the two exceptions 0.3 already had.

## Codegen and backends

Three emitters selected by `cell emit --target=`:

- `c` (default): only one that lowers the whole language today (if/else, match, blocks, struct/list literals, mangled calls). Walks AST directly. **Direction, ruled 2026-09-21:** `.cell -> HIR -> LLVM IR/MLIR` is the primary pipeline and C is emitted only where it is still needed; `c` stays the default until every LLVM/MLIR leak pin in gate stage 7 reads 0, then the default flips to `llvm`.
- `llvm`, `mlir`: go through `hir`; deliberately **scalar-first**. Refuse most of `String`, `[T]`, any `T?` or `Result` beyond scalar `Some`/`None`/`Ok`/`Err` payloads, `arc` (and most aggregates crossing the C boundary) with a `cannot lower` diagnostic at the span. Never emit plausible wrong code. **Two exceptions, both narrower than the blanket claim this line used to make:** an `exclusive` `String`/`[T]`/`T?` PARAMETER and a whole-value write through it now lower in both backends, since `a6c41e8` made them pointers matching `abi.classifyParam` and the C ABI, and since 2026-09-17 MLIR places every parameter and return by `abi.classifyParam`/`classifyReturn` exactly as `llvmemit.zig` does; and, since IR String step (a) (2026-09-17), owned `String` values: a place reads as the 24-byte value it is, a borrowed view of it is a `string_view` HIR node, and the `str` -> owning-`String` conversion is a `cell_string_from_str` call that `hir.lower`'s `convertTo` funnel inserts at every declared destination C converts at, declaring the helper once through `hir.runtime_callees`. An unannotated `let owned s = "ab"` (C keeps it a view) and a binding pattern over a view are still refused by the emitters' `fits` guard. Neither backend has a drop pass, so no owned String either builds is freed; `examples/leaks/ir_owned_string.cell` and `ir_string_conversion.cell` pin that in gate stage 7. `examples/owned_string.cell` prints 44 on all three backends.
...
- `src/cell/`: stages live here (`load`, `lexer`, `parser`, `ast`, `typecheck`, `borrowck`, `hir`, `codegen` (AST→C), `llvmemit`, `mlirmit`, `diag`...).
  **Layout:** `borrowck.zig` and `codegen.zig` are roots over `src/cell/borrowck/` and `src/cell/codegen/` (split as pure moves, `bbe3b13..84dde77`). Each root keeps its module header, the public surface, and the `comptime` block that pulls in the directory's `tests_*.zig`, so `zig test src/cell/borrowck.zig` still runs every borrowck test; most of the checking and lowering code lives in the directory files (`codegen.zig` keeps the drop pass), so cite a moved symbol by its file there. `borrowck.zig`'s `//!` header must stay in `borrowck.zig`: `tools/check-rule-lists.sh` reads the enforced-rule list from that file's leading `//!` block.
...
Stem pairing lives in `load.zig`, and its diagnostics are its own, NOT `docs/OWNERSHIP.md` rules: missing module file, ambiguous module (both `.cell` and `.cel` exist), a declaration that already has a body, and body-versus-declaration signature mismatch (parameter count, per-parameter ownership and type, return type). No R-numbered rule is checked there. R8 is the escaping-borrow rule in `borrowck.zig`. R11 is `arc` retain-release: implemented in `codegen.zig`'s C backend with the gaps listed above, not in `llvmemit.zig` or `mlirmit.zig`, which refuse `arc`. R10 (move-into-`arc`) is implemented for one source shape only, a whole `owned` `String` or list binding (`boxableOwnedBinding`), at the five positions listed under Status honesty above, and refused as not implemented for every other source and position; see `borrowck.zig`'s header. (Corrected 2026-09-21: this line said R10's move-into-`arc` was designed, not implemented, which stopped being true on 2026-09-16.)

## Conventions

Match the Zig already present (master idioms, at the master revision `build.zig.zon` pins): `std.ArrayList(T) = .empty`, allocator-passing methods, `std.Io`, `pub fn main(init: std.process.Init)`, etc. Do not rewrite toward older forms.

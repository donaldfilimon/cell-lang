    ...
    fn checkExpr(self: *Checker, expr: *ast.Expr) CheckError!Type {
        switch (expr.kind) {
                ...
                for (m.arms) |arm| {
                    ...
                    switch (arm.pattern.kind) {
                        ...
                        .wrap_pattern => |wp| {
                            ...
                            // Sub-projects 2-4 (2026-09-17): an owning String
                            // payload in Ok, Err, or Some.
                            const owning = wp.ctor != .none and known and payload.?.tag() == .string;
                            ...
                            if (wp.mode) |mode| {
                                ...
                                if (wp.binding) |name| {
                                    const ctor_name: []const u8 = switch (wp.ctor) {
                                        .ok => "Ok",
                                        .err => "Err",
                                        .some, .none => "Some",
                                    };
                                    try self.errf(arm.pattern.span, "'{s}({s})' on an owning payload must say 'owned' or 'shared'", .{ ctor_name, name });
                                    mode_ok = false;
                                }
                            }
                            if (wp.binding) |name| {
            ...
            .wrap => |w| {
                ...
                switch (w.ctor) {
                    .some => {
                        // An owning String is admitted (sub-project 4).
                    ...
                    .err => {
                        ...
                        // it must be a scalar, a payload-free enum, or an
                        // owning String (sub-project 3, 2026-09-17).
                        if (!isScalarPayload(payload) and payload.tag() != .enum_type and payload.tag() != .string) {
                            try self.errf(expr.span, "optional/Result payloads other than scalar primitives are not implemented", .{});
                            return try self.resultOf(types.t_unknown, types.t_unknown);
                        }
...
test "an owning String error is built by Err and bound with owned or shared" {
    // Owning String in Err (sub-project 3, 2026-09-17).
    var t: TestModule = .init();
    defer t.deinit();
    try t.check(
        \\pub fn mk(owned s: String) -> Result<Int, String> {
        \\    return Err(s)
        \\}
        \\pub fn a(owned r: Result<Int, String>) -> Int {
        \\    return match r { Ok(v) => v, Err(owned e) => 1 }
        \\}
        \\pub fn b(owned r: Result<Int, String>) -> Int {
        \\    return match r { Ok(_) => 0, Err(shared e) => 2 }
        \\}
        \\pub fn c(owned r: Result<String, String>) -> Int {
        \\    return match r { Ok(owned x) => 1, Err(shared e) => 2 }
        \\}
    );
    try t.expectClean();
    try expectOnlyMessage(
        \\pub fn f(owned r: Result<Int, String>) -> Int {
        \\    return match r { Ok(v) => v, Err(e) => 2 }
        \\}
    , "'Err(e)' on an owning payload must say 'owned' or 'shared'");
    try expectOnlyMessage(
        \\pub fn f(owned r: Result<String, String>) -> Int {
        \\    return match r { Ok(x) => 1, Err(owned e) => 2 }
        \\}
    , "'Ok(x)' on an owning payload must say 'owned' or 'shared'");
}

test "an owning String? is built by Some and bound with owned or shared" {
    // Sub-project 4 (2026-09-17).
    var t: TestModule = .init();
    defer t.deinit();
    try t.check(
        \\pub fn mk(owned s: String) -> String? {
        \\    return Some(s)
        \\}
        \\pub fn a(owned o: String?) -> Int {
        \\    return match o { Some(owned x) => 1, None => 0 }
        \\}
        \\pub fn b(owned o: String?) -> Int {
        \\    return match o { Some(shared x) => 1, None => 0 }
        \\}
        \\pub fn c(owned o: String?) -> Int {
        \\    return match o { Some(_) => 1, None => 0 }
        \\}
    );
    try t.expectClean();
    try expectOnlyMessage(
        \\pub fn f(owned o: String?) -> Int {
        \\    return match o { Some(s) => 1, None => 0 }
        \\}
    , "'Some(s)' on an owning payload must say 'owned' or 'shared'");
}
...
test "a mode is refused where it means nothing" {
    ...
    try expectOnlyMessage(
        ...
        \\}
    , "a scalar payload is copied; remove 'shared'");
    try expectOnlyMessage(owning_prelude ++
        \\pub fn f(owned r: Result<String, Int32>) -> Int {
...
test "scalar payloads only" {
    // `Some` of a String is admitted since sub-project 4 (2026-09-17); a
    // struct payload is still refused.
    var t: TestModule = .init();
    defer t.deinit();
    try t.check(
        \\pub struct P { copy a: Int }
        \\pub fn f(copy p: P) -> P? {
        \\    return Some(p)
        \\}
    );
    try t.expectCount(1);
    try t.expectDiag(0, .err, 3, 12, "optional/Result payloads other than scalar primitives are not implemented");
}

...
| `field_revival.cell` | R16 residual: a field revived after it was moved leaked the new value; **CLOSED 2026-09-16** by retracting the revived path from `fieldWasMoved` (`src/cell/borrowck/scope.zig`), 1000 -> 0 |
...
| `discarded_result.cell` | not a numbered row: an owned String call result discarded in statement position is freed by no backend; 2000 (1000 calls, two results), disclosed 2026-09-17. **CLOSED 2026-09-21 in C:** 0; a discarded owned String call result is bound to a temporary and released at once (question 2 of the drop-pass spec, ruled fix-now). LLVM/MLIR still leak it: they have no drop pass |

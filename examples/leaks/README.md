...
| `owned_scalar_wrappers.cell` | not an R11 row: owned scalar `Result<T, E>` and `T?` values built, passed `owned`, returned, reassigned and dropped 1000 times. They hold no heap memory (scalar `T`, `E` narrowed to an `int32_t` code), so the count is 0 on both witnesses; **pinned at 0 2026-09-17** so a future resource-bearing payload surfaces here instead of leaking silently |

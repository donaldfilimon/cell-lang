...
| TYPE-06 generic types/functions and typed Result | partial | partial | refused | refused | Requires typed owning payloads. Since 2026-09-16 `Result<T, E>` parses and checks and C passes it through as `cell_result_t`; no construction or inspection syntax, no drop (an owned Result leaks), other generics refused at parse | [generics](../examples/future/generics.cell), [Result](../examples/future/result.cell) | M6 |
...
| CLI-02 build/run/test and core SDK | partial | partial | refused | refused | `cell build`/`cell run` landed 2026-09-16 (C target only; `$CC` on the emitted C plus an embedded copy of `runtime/cell_rt.{h,c}`, with `.c` positionals passed through as hand-written hosts, so `examples/arc.cell` and `examples/owned_string.cell` run through the CLI too). `test` and the core SDK remain absent: prelude group 3 has no implementation, and `build` now shows that concretely as an undefined symbol at link time | [CLI](../src/main.zig), [gate stage 12](../tools/check.sh), [prelude](../stdlib/prelude.cell) | M9 |

...
| FLOW-01 if/block/match values | checked | lowered | partial | partial | Owning branches constrained by move checker; an `if` branch that always leaves keeps its moves out of the code after it since 2026-09-17 ([early return](../examples/early_return.cell)); `match` arms still merge | [control flow](../examples/control_flow.cell) | M3/M4 |

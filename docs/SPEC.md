...
and R10's move-into-`arc` is implemented only at `let` for a whole `owned`
`String` or list binding (refused elsewhere). The C
...
and implemented at `let` for a whole binding on 2026-09-16, which empties
R11's table).
...
land; move-into-`arc` is implemented only at `let`, for a whole `owned`
`String` or list binding (2026-09-16). Every `arc`-to-`arc` use stays

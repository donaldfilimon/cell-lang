...
| Drop insertion for `owned` | partially implemented: unmoved `owned`/`arc` `let`/`var` locals, block-scoped for statement-position scopes and value-position blocks, conservative on moves; structs through per-struct drop glue (a field moved on only one branch or moved only in part is left unreleased); `owned`/`arc` parameters; a `var` revived after a move at a block end, `return`, `break`/`continue`, value-block end, and a revived record (see `docs/OWNERSHIP.md` R16). A var moved inside a `while` it was declared outside of still leaks |

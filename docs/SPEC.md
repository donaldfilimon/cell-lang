...
and AddressSanitizer measurements; **four of the six are CLOSED** (the last
three dated 2026-09-15; the gate's leaks stage pins each at 0 and is the
authority): an `arc` parameter is never released by a Cell body, because no
parameter is dropped; a struct holding an `arc` field used to be never

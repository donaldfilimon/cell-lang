...
- **Drop helper.** Add one per instance, next to the instance, generated by the same X-macro:

  ```c
  static inline void cell_res_string_<err>_drop(cell_res_string_<err>_t *r) {
      if (r->ok) cell_string_free(&r->as.ok);
  }
  ```

  - The helper lives in the header rather than as generated glue **(default)**. The instance is already predefined there, and a host gets it for free.
...
- **Pattern modes.** For a wrap binding:
  ...
  - **`Ok(owned _)` / `Ok(shared _)`:** refused ("a wildcard binds nothing") **(default)**. `Ok(_)` is accepted and neither moves nor borrows.
...
- **Drops.**
  ...
  - `emitDropFor` gains a `.result` arm that calls `cell_res_string_<err>_drop(&x)`.
...
1. Is the refusal of any mode on `Some`/`Err`/scalar payloads right, or should `Err(owned e)` parse now and wait for sub-project 3 in the checker only (which this design already does)?
2. Should `Ok(_)` on an owning payload be accepted (no move, released after the match), or refused so every arm says what it does with the String?
3. Is the drop helper in `cell_rt.h` acceptable, or should it be generated glue like `cell_drop_<Name>`?
